  ...
  campaignInfo_per_route=function(adwordsFile=NA,bingFile=NA,convColumnName='Conversions',consider_direction=F, create.csv=F){
    ...
    #-----------------------------------bing data
    if(!is.na(bingFile)){
      print(paste("1. Getting data from Bing."))
      bing=subset(subset(read.csv(bingFile),Campaign.ID !="-"),select = c('Month','Campaign','Spend','Impr.','Clicks','Conv.'))
      head(bing)
      colnames(bing)=c('Date','Campaign','Cost','Impr.','Clicks','Ads_Conv')
      bing$Month=as.character(format(as.Date(bing$Date,format='%d/%m/%Y'),'%m'))
      bing$Year=as.character(format(as.Date(bing$Date,format='%d/%m/%Y'),'%Y'))
      bing$Campaign=as.character(bing$Campaign)
      bing$Cost=as.numeric(gsub(',','',as.character(bing$Cost)))
      bing$Impr.=as.numeric(gsub(',','',as.character(bing$Impr.)))
      bing$Clicks=as.numeric(gsub(',','',as.character(bing$Clicks)))
      bing$Campaign[which(bing$Campaign=="DAYTRIP brand")]='Daytrip to Daytrip xx-xx' #"DAYTRIP brand plošně"
      bing$Campaign=gsub(' - ',' ',bing$Campaign)
      
      campaignName=function(x){
        x=gsub(' W','',gsub('/ ','',gsub('\\d','',x)))
        if(regexpr('-', x)!=-1){ x=substr(x,1,regexpr('-', x)-4)}
        if(consider_direction==F){  return(paste(trimws(sort(strsplit(x,' to ')[[1]]),which='both'),collapse=' - ')) }
        if(consider_direction==T){  return(paste(trimws(    (strsplit(x,' to ')[[1]]),which='both'),collapse=' to ')) }
      }
      bing$campaignName=unlist(lapply(bing$Campaign, campaignName))
      bing=subset(bing,select=c("Cost","Impr.","Clicks","Ads_Conv","Month","Year","campaignName"))
      bing$Google_search_impr=NA
      #head(bing)
    }
  ...
  campaignInfo_per_country=function(adwordsFile=NA,bingFile=NA,convColumnName='Conversions',consider_direction=F, create.csv=F){
    ...
    if(!is.na(bingFile)){
      ...
      bing$Impr.=as.numeric(gsub(',','',as.character(bing$Impr.)))
      bing$Clicks=as.numeric(gsub(',','',as.character(bing$Clicks)))
      bing$Campaign[which(bing$Campaign=="DAYTRIP brand")]='Daytrip to Daytrip xx-xx'
      
      x=bing$Campaign[196]
      campaignName=function(x){
        x=trimws(gsub(' OLD','',gsub(' W','',gsub('/ ','',gsub('\\d','',x)))),'both')
        x=substr(x,nchar(x)-4,nchar(x))
        
        if(regexpr('-', x)==-1){x='NA-NA'}
        if(consider_direction==F){  return(paste(trimws(sort(strsplit(x,'-')[[1]]),which='both'),collapse=' - ')) }
        if(consider_direction==T){  return(paste(trimws(    (strsplit(x,'-')[[1]]),which='both'),collapse=' to ')) }
      }
